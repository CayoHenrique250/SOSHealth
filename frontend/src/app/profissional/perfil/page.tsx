...
export default function PerfilProfissional() {
  ...
  useEffect(() => {
    ...
    reset({
      ...
      specialty: data.personal.specialty || "Clínica Geral",
      ...
      <section className="mx-auto flex w-full max-w-5xl flex-1 flex-col items-start px-6 py-10">
        ...
        <div className="grid w-full items-start gap-10 lg:grid-cols-[1fr_minmax(280px,360px)]">
          <form onSubmit={handleSubmit(onSubmit)} className="rounded-3xl bg-[#1e3e44] p-6 shadow-md">
            {activeTab === "pessoais" ? (
              <div className="space-y-1">
                ...
                <div className="mb-2">
                  <label className={`mb-1 ml-1 block text-sm font-bold ${labelDark}`}>Especialidade :</label>
                  <select {...register("specialty")} className="w-full rounded-lg border-2 border-white/30 bg-[#1e3e44] px-3 py-2 text-sm font-medium text-white focus:border-white focus:outline-none focus:ring-2 focus:ring-[#6ba2a6]">
          ...
          <div className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm">
            ...
            <AvatarFallback name={data?.personal.name} src={data?.personal.avatar} className="h-80 w-full rounded-2xl object-cover lg:h-96" />
            <button
              type="button"
