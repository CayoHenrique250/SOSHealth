    ...
    <main className="min-h-screen flex flex-col bg-[#f3f5f5]">
      ...
      <section className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 py-6 sm:py-10">
        ...
        <div className="mt-6 sm:mt-8 rounded-2xl bg-white p-3 sm:p-4 shadow-sm">
          <h2 className="mb-3 text-base sm:text-lg font-semibold text-slate-800">Calendários rápidos</h2>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {["2026-05", "2026-06", "2026-07"].map((key) => (
              <article key={key} className="rounded-xl border border-slate-100 p-3">
                <p className="mb-2 text-xs sm:text-sm font-semibold text-slate-800">{key}</p>
                <div className="grid grid-cols-7 gap-1 text-[9px] sm:text-[10px]">
                  {Array.from({ length: 14 }, (_, i) => i + 1).map((day) => {
                    ...
                    return <span key={`${key}-${day}`} className={`rounded py-0.5 sm:py-1 text-center ${color} min-h-[18px] sm:min-h-[20px] flex items-center justify-center`}>{day}</span>;
                  })}
                </div>
              </article>
            ))}
